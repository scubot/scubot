...
class CustomHelpCommand(commands.HelpCommand):
	...
	async def send_command_help(self, command):
		emb = discord.Embed(title="Scubot Help", description=f"```{self.get_command_signature(command)}```", colour=0x008080)
	...
	async def send_group_help(self, group):
		emb = discord.Embed(title="Scubot Help", description=f"Help for `{group.name}` command group.\nUse `{self.clean_prefix}help {group.name} [command]` for more info on a command.",colour=0x008080)
		emb.add_field(name="Details", value=group.help or "No details available.", inline=False)
		emb.add_field(name="Available Subcommands", value=f"```\n{', '.join([command.name for command in group.commands])}\n```", inline=False)
		await self.context.send(embed=emb)
