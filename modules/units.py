...
class Units(commands.Cog):
		...
		if amount is None and unit is None:
			...
			for message in channel_hist:
				...
				if message_parts[0].isdigit():
					if message_parts[1] in SYMBOL_MAP.keys() or message_parts[1] in SYMBOL_MAP.values():
						converted_unit = self.convert_unit(float(message_parts[0]), message_parts[1])
						return await ctx.send(converted_unit)
			return await ctx.send("[!] No valid conversions found in the last 10 messages.")
			...
			for message in channel_hist:
				...
				if message_parts[0].isdigit():
					if message_parts[1] in SYMBOL_MAP.keys() or message_parts[1] in SYMBOL_MAP.values():
						output.append(self.convert_unit(float(message_parts[0]), message_parts[1]))
			if not output:
				return await ctx.send("[!] No valid conversions found in the last 10 messages.")
			else:
				output = "\n".join(output)
			...
			if amount is None:
				...
				try:
					...
					try:
						converted_unit = self.convert_unit(float(amount), SYMBOL_MAP[unit])
					except KeyError:
	...
	async def units(self, ctx):
		...
		embed.description = "\n".join((f"{k} = {v}" for k, v in (SYMBOL_MAP.keys(), SYMBOL_MAP.values())))
